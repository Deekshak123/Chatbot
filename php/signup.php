   	...
   	if(filter_var($email, FILTER_VALIDATE_EMAIL)){
         ...
         if(mysqli_num_rows($sql) > 0){//if email already exist
            ...
            if(isset($_FILES['image'])){//if file is uploaded
               ...
               //let's explode image and get extension like jpg png
               ...
               if(in_array($img_ext, $extensions) === true){//if user uploaded img ext matched with any array extensions
                  $time = time();//this will return us current time
                                 //we need this time because when you uploading user img to in our folder we rename user file with current time so all the img file will have unique name
                  ...
                  if(move_uploaded_file($tmp_name, "images/".$new_img_name)){//if user upload img move to our folder successfully
                       ...
                       if($sql2){
                        ...
                        if(mysqli_num_rows($sql3) > 0){
                           ...
                           $_SESSION['unique_id'] = $row['unique_id'];//using this session we used user unique_id in othr php file
                           echo "success";
                        }
                       }else{
